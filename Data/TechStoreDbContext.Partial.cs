...
{
    /// <summary>
    /// Hand-written half of <see cref="TechStoreDbContext"/>. Kept in a SEPARATE file on purpose:
    /// TechStoreDbContext.cs is regenerated every time you run
    ...
    ///    <c>UI/AppState.cs</c>, which does <c>using var db = new TechStoreDbContext();</c> once per
    ///    operation).
    ...
    ///    TechStoreDbContext.cs (scaffold warns about this on the console when you omit
    ///    <c>--no-onconfiguring</c> — the option this project always scaffolds with).
    /// </summary>
    public partial class TechStoreDbContext
    {
